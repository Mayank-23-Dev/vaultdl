...
import {
  ...
  Check, ArrowRight, FileVideo, Instagram, Loader2, AlertCircle, Zap,
} from "lucide-react";
...
export function DownloaderPage() {
  ...
  const youtubeCount = items.filter(i => i.url.includes("youtube.com") || i.url.includes("youtu.be")).length;
  const instagramCount = items.filter(i => i.url.includes("instagram.com")).length;
  ...
  const [selectedFormat, setSelectedFormat] = useState<"video" | "audio" | "thumbnail">("video");
  ...
  async function handleDownload() {
    ...
    try {
      ...
      await startDownload(url, fmt, selectedQuality, selectedFormat, videoInfo.title, videoInfo.thumbnail);
              ...
              <div className="grid grid-cols-3 gap-2">
                {([
                  ...
                  <button
                    ...
                    className={cn(
                      "flex flex-col items-center gap-2 py-4 px-2 rounded-xl border transition-all duration-200 text-center relative overflow-hidden",
                      selectedFormat === id
                  ...
                  >
                    ...
                    {selectedFormat === id && (
                      <Check className="w-3 h-3 text-white/50 -mt-0.5" strokeWidth={2.5} />
                    )}
                  </button>
                ))}
              </div>
          ...
          <div className="bg-[rgb(9_9_12)] border border-white/[0.07] rounded-2xl p-5">
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-4">
              {[
                ...
                { label: "Format",       value: selectedFormat,                          hi: true  },
