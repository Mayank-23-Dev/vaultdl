...
import { Search, Download, FolderOpen, Trash2, RefreshCw, AlertCircle, Clock, Copy, Check } from "lucide-react";
...
export function HistoryPage() {
  ...
  const handleCopyLink = useCallback((url: string, id: string) => {
    ...
    <div className="p-6 max-w-[1100px] mx-auto">
      ...
      {!loading && filtered.length > 0 && (
        <div className="bg-[rgb(9_9_12)] border border-white/[0.07] rounded-2xl overflow-hidden">
          ...
          {/* Table header */}
          <div className="grid grid-cols-[2fr_1fr] md:grid-cols-[2fr_100px_80px_80px_140px_64px] px-5 py-2.5 border-b border-white/[0.05]">
            <div className="text-[9px] font-semibold uppercase tracking-[1.4px] text-white/20">Title</div>
            <div className="md:hidden text-[9px] font-semibold uppercase tracking-[1.4px] text-white/20 text-right">Info</div>
            {["Platform", "Format", "Quality", "Date", ""].map(col => (
              <div key={col} className="hidden md:block text-[9px] font-semibold uppercase tracking-[1.4px] text-white/20">
                {col}
              </div>
            ))}
          </div>
          ...
          {filtered.map((item, idx) => {
            ...
            return (
              <div
                key={item.id}
                className={cn(
                  "grid grid-cols-[2fr_1fr] md:grid-cols-[2fr_100px_80px_80px_140px_64px] items-center px-5 py-3 group transition-colors hover:bg-white/[0.02]",
              ...
              >
                ...
                <div className="md:hidden flex flex-col items-end gap-1">
                   <div className="flex items-center gap-2">
                     <button
                        className="w-7 h-7 flex items-center justify-center rounded-lg text-white/25 hover:text-white/60 bg-white/[0.03]"
                        onClick={() => {
                          const basePath = item.type === 'audio' ? '~/Downloads/VaultDL/Audio' : item.type === 'thumbnail' ? '~/Downloads/VaultDL/Thumbnails' : '~/Downloads/VaultDL/Videos';
                          window.electron?.openFolder(basePath);
                        }}
                      >
                        <FolderOpen className="w-3.5 h-3.5" />
                      </button>
                      <button
                        className="w-7 h-7 flex items-center justify-center rounded-lg text-white/25 hover:text-red-400/70 bg-white/[0.03]"
                        onClick={() => handleDelete(item.id)}
                      >
                        <Trash2 className="w-3.5 h-3.5" />
                      </button>
                   </div>
                   <span className="mono text-[10px] text-white/20 uppercase">{item.format || "mp4"} · {item.quality || "best"}</span>
                </div>
                ...
                {/* Actions (Desktop) */}
                <div className="hidden md:flex items-center gap-0.5 justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                  ...
                  <button
                    ...
                    title="Open folder"
                    onClick={() => {
                      // Attempt to open the folder based on the download path logic in server.mjs
                      const basePath = item.type === 'audio' ? '~/Downloads/VaultDL/Audio' : item.type === 'thumbnail' ? '~/Downloads/VaultDL/Thumbnails' : '~/Downloads/VaultDL/Videos';
                      window.electron?.openFolder(basePath);
                    }}
                  >
